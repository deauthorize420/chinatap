...
local ffi = ffi
local FONT_URLS = {
    { file = "chinatap_Oxanium.ttf",      url = "https://cdn.jsdelivr.net/gh/google/fonts@main/ofl/oxanium/Oxanium%5Bwght%5D.ttf" },
...
local function initFonts()
    ...
    print("[chinatap] font: " .. tostring(picked))
end
...
local function fontInitCoro()
    ...
    if gdi32 then
        ...
        print("[chinatap] ffi/gdi32 unavailable, using system fonts")
    end
...
function Section:_widget(wd, x, y, w)
    ...
    elseif wd.kind == "button" then
        ...
        if clicked(x, y + 1, w, bh) then
            local ok, err = pcall(wd.cb); if not ok then print("[chinatap] button error: " .. tostring(err)) end
        end
    ...
    elseif wd.kind == "custom" then
        if wd.fn then
            ...
            if not ok then print("[chinatap] custom widget error: " .. tostring(err)) end
            local used = UI._cy - y
...
local function renderSectionAt(s, x, y, w)
    ...
    if not ok then print("[chinatap] section '" .. tostring(s.title) .. "' error: " .. tostring(err)); return h end
    return rh
end
...
function M:_frame()
    ...
    if tab then
        ...
        if not ok then print("[chinatap] tab '" .. tostring(tab.name) .. "' error: " .. tostring(err)) end
    end
...
function M:Build(opts)
    ...
    if not _getMouse then print("[chinatap] WARNING: mouse position API not found -- cursor won't track") end
    ...
    callbacks.Register("Draw", function()
        ...
        if not ok then print("[chinatap] frame error: " .. tostring(err)) end
    end)
