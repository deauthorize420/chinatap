local BASE        = rawget(_G, "CHINA_BASE") or "https://raw.githubusercontent.com/deauthorize420/chinatap/main/"
...
local function load(url, cacheFile, name)
    ...
    if not src then print("[chinatap] FATAL: cannot load " .. name) return nil end
    local chunk, err = loadstring(src, "=" .. cacheFile)
    if not chunk then print("[chinatap] " .. name .. " compile error: " .. tostring(err)) return nil end
    local ok, mod = pcall(chunk)
    if not ok then print("[chinatap] " .. name .. " run error: " .. tostring(mod)) return nil end
...
do
    ...
    local function install()
        if type(ffi) ~= "table" then print("[chinatap] VM: no ffi"); return false end
        ...
        if not a or a == 0 then print("[chinatap] VM: sig not found"); return false end
        match = a
        local orig = a + 5 + r_i32(a + 1)

        local p = alloc_near(orig, 0x1000)
        if p == nil then print("[chinatap] VM: alloc failed"); return false end
        ...
        if rel < -2147483648 or rel > 2147483647 then print("[chinatap] VM: rel32 overflow"); return false end
...
do
    ...
    if type(f) == "table" then
        ...
        RG.ok = okI
        if okI then print("[chinatap] region: hooked " .. #hooks .. " fns (" .. #RG.ids .. " pops)")
        else            print("[chinatap] region: hook failed") end
    end
...
do
    ...
    local function install()
        if type(f) ~= "table" then print("[chinatap] namechanger: no ffi"); return false end
        local a = mem.FindPattern(DLL, SIG_SETINFO)
        if not a or a == 0 then print("[chinatap] namechanger: sig not found"); return false end
        T = a
        local b0 = f.cast("uint8_t*", T)
        local p = alloc_near(T); if p == nil then print("[chinatap] namechanger: alloc failed"); return false end
        local TR = tonumber(f.cast("uintptr_t", p))
        ...
        if f.C.VirtualProtect(f.cast("void*", T), STEAL, 0x40, old) == 0 then
            print("[chinatap] namechanger: protect failed"); return false
        end
    ...
    function NC.dump()
        ...
        print("[chinatap] NC: base=" .. hx(d.base) .. " cvar=" .. hx(d.cvar) ..
    ...
    pcall(function() okI = install() end)
    NC.ok = okI
    if okI then print("[chinatap] namechanger: hooked SetInfo @ " .. string.format("%X", T))
    else        print("[chinatap] namechanger: install failed") end
end
...
do
    ...
    if type(f) == "table" then
        ...
        if a and a ~= 0 then
            ...
            CHAT.ok = true
            print("[chinatap] chat: hooked print @ " .. string.format("%X", a))
        else
            print("[chinatap] chat: print sig not found")
        end
    end
...
do
    ...
    local function pfx() return "[" .. P .. "chinatap" .. W .. "] " end
...
pcall(function()
    ...
    callbacks.Register("FireGameEvent", "ChinaTap_Events", function(ev)
        pcall(HS.onEvent, ev)
        pcall(VR.onEvent, ev)
    end)
end)
...
local NC_FEM = {
    { t = "",          ms = 550 },
    { t = "$C",         ms = 55 },  { t = "$c",         ms = 85 },
    { t = "$cH",        ms = 55 },  { t = "$ch",        ms = 85 },
    { t = "$ch1",   ms = 55 },  { t = "$chi",       ms = 85 },
    { t = "$chi|/|",      ms = 55 },  { t = "$chin",      ms = 85 },
    { t = "$chin4",     ms = 55 },  { t = "$china",     ms = 85 },
    { t = "$chinA",    ms = 55 },  { t = "$china",    ms = 85 },
    { t = "$chinaT",   ms = 55 },  { t = "$chinat",   ms = 85 },
    { t = "$chinat@",  ms = 55 },  { t = "$chinata",  ms = 85 },
    { t = "$chinataP", ms = 55 },  { t = "$chinatap", ms = 90 },
    { t = "$chinatap",  ms = 70 }, { t = "$chinatap$", ms = 2000 },
    { t = "$chinatap",  ms = 70 }, { t = "$chinatap",   ms = 70 },
    { t = "$chinata",  ms = 60 },  { t = "$chinat",   ms = 60 },
    { t = "$china",    ms = 60 },  { t = "$china",     ms = 60 },
    { t = "$chi",      ms = 60 },  { t = "$chi",       ms = 60 },
    { t = "$ch",        ms = 60 },  { t = "$c",         ms = 60 },
}
...
local NC_FEM_G = ncGlitch("$chinatap$")
...
ncSrc    = ncSec:Combo("Source", { "Static", "Chinatap", "Aimware", "Custom" }, 1)
