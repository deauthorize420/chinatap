...
local src, where = fetch(BASE .. "chinatap.lua", ".\\chinatap_lua\\chinatap.lua")
if not src then print("[loader] FATAL: cannot fetch chinatap.lua") return end

local chunk, err = loadstring(src, "=chinatap.lua")
if not chunk then print("[loader] compile error: " .. tostring(err)) return end

_G.FEMBOY_BASE = BASE
print(string.format("[loader] chinatap %s from %s", ref(), tostring(where)))
